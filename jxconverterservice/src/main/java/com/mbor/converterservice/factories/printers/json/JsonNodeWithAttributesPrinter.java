...
import com.mbor.converterservice.utils.JsonUtils;
    ...
    public String prepareElement(AbstractNode abstractNode) {
        ...
        for (Map.Entry<String, String> entry : node.getAttributes().entrySet()) {
            ...
            stringBuilder.append(JSON_QUOTE).append(ATTRIBUTE_SIGN).append(entry.getKey()).append(JSON_QUOTE).append(EMPTY_SPACE).append(JSON_COLON).append(EMPTY_SPACE)
                    .append(JSON_QUOTE).append(entry.getValue()).append(JSON_QUOTE);
            stringBuilder.append(CommonUtils.NEW_LINE);
        }
        stringBuilder.append(JSON_QUOTE).append(ELEMENT_SIGN).append(node.getNodeName()).append(JSON_QUOTE).append(EMPTY_SPACE).append(JSON_COLON).append(EMPTY_SPACE);
